...
describe("HelpRequestIndexPage tests", () => {
    ...
    test("Renders with Create Button for admin user", async () => {
        ...
        expect(button).toHaveAttribute("href", "/ucsbhelprequest/create");
        expect(button).toHaveAttribute("style", "float: right;");
    });

    test("renders three help requests correctly for regular user", async () => {
        setupUserOnly();
        axiosMock.onGet("/api/ucsbhelprequest/all").reply(200, helpRequestFixtures.threeHelpRequests);
    ...
    test("renders empty table when backend unavailable, user only", async () => {
        ...
        expect(errorMessage).toMatch("Error communicating with backend via GET on /api/ucsbhelprequest/all");
        restoreConsole();
    ...
    test("what happens when you click delete, admin", async () => {
        ...
        axiosMock.onGet("/api/ucsbhelprequest/all").reply(200, helpRequestFixtures.threeHelpRequests);
        axiosMock.onDelete("/api/ucsbhelprequest").reply(200, "Help request with id 1 was deleted");
        ...
        expect(axiosMock.history.delete[0].url).toBe("/api/ucsbhelprequest");
        expect(axiosMock.history.delete[0].url).toBe("/api/ucsbhelprequest");
